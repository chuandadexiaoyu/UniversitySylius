...
class BackendMenuBuilder extends MenuBuilder
{
    ...
    protected function addConfigurationMenu(ItemInterface $menu, array $childOptions, $section)
    {
        ...
        $child->addChild('china_provinces', array(
            'route' => 'sylius_backend_china_province_index',
            'labelAttributes' => array('icon' => 'glyphicon glyphicon-globe'),
        ))->setLabel($this->translate(sprintf('sylius.backend.menu.%s.china_provinces', $section)));

        $child->addChild('cities', array(
            'route' => 'sylius_backend_city_index',
            'labelAttributes' => array('icon' => 'glyphicon glyphicon-globe'),
        ))->setLabel($this->translate(sprintf('sylius.backend.menu.%s.cities', $section)));

        $child->addChild('universities', array(
            'route' => 'sylius_backend_university_index',
            'labelAttributes' => array('icon' => 'glyphicon glyphicon-globe'),
        ))->setLabel($this->translate(sprintf('sylius.backend.menu.%s.universities', $section)));

        $child->addChild('campus', array(
            'route' => 'sylius_backend_campus_index',
            'labelAttributes' => array('icon' => 'glyphicon glyphicon-globe'),
        ))->setLabel($this->translate(sprintf('sylius.backend.menu.%s.campus', $section)));
    }
}
